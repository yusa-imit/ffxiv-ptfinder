  ...
  return (
    ...
    >
      <article style={{ width: '100%', position: 'relative' }}>
        <Stack>
          <Section title={t('article_view_section_title_title')}>
            <Title order={1} style={{ wordBreak: 'break-word', lineBreak: 'auto' }}>
              {article.title}
            </Title>
          </Section>
          <Section title={t('article_view_section_defaults_title')}>
            {article.isTemporary && (
              <Group>
                <ArticleBadge>{t('article_view_isTemporary_badge')}</ArticleBadge>
              </Group>
            )}
            {
              // TODO
            ...
            <Group>
              <ArticleBadge>{`${t('article_view_region')} : ${article.region}`}</ArticleBadge>
              <ArticleBadge>{`${t('article_view_language')} : ${article.language}`}</ArticleBadge>
            </Group>
          </Section>
          <Section title={t('article_view_section_jobs_title')}>
            <SubTitle>{t('article_view_section_jobs_job_all')}</SubTitle>
            {article.jobs.flat(3).length === 0 ? (
              <Text>{t('article_view_section_jobs_job_not_specified')} </Text>
            ) : (
              <Group>
            ...
            <SubTitle>{t('article_view_section_jobs_job_position')}</SubTitle>
            <Stack>
                ...
                <Group key={partyIndex}>
                  {party.map((position, positionIndex) => (
                    <Tooltip
                      label={
                        position.length === 0
                          ? t('article_view_section_jobs_job_not_specified')
          ...
          <Section title={t('section_details_title')}></Section>
          <Section title={t('section_schedules_title')}></Section>
