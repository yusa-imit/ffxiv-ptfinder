...
import { runBasicTests } from './adapterTest';
...
runBasicTests({
  ...
  db: {
    ...
    async verificationToken({ identifier, token }) {
      const snapshotQuery: SqlQuerySpec = {
        query: 'select * from tokens v where v.identifier=@identifierValue and v.token=@tokenValue',
        parameters: [
      ...
      throw new Error('No exact matching token.');
    },
  },
});
