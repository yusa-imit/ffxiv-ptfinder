...
async function setDb(clientOptions: CosmosClientOptions, dbOptions: CosmosDataBaseOptions) {
  db = (
    await getClient(clientOptions).databases.createIfNotExists(dbOptions.body, dbOptions.options)
  ).database;
}
